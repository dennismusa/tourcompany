...
<nav
className={`fixed top-0 left-0 w-full z-[100] border-b border-white/10 backdrop-blur-xl transition-all duration-500 ${
scrolled
?"bg-[#151515] py-2"
:"bg-[#151515] py-4"
}`}
...
<img
src={safarilinklogo}
className="w-11 h-11 rounded-full border border-yellow-400 object-cover"
/>
...
<div
className={`dropMenu right-0 w-40 ${
langOpen
?"show"
:"hide"
}`}
...
<div
onClick={closeMobile}
className={`fixed inset-0 bg-black/70 backdrop-blur-sm z-[180] transition-all duration-500 md:hidden ${
menuOpen
?"opacity-100 visible"
:"opacity-0 invisible"
}`}
...
<div
className={`fixed top-0 right-0 h-screen w-full bg-[#0d0d0d] z-[200] transition-all duration-500 overflow-y-auto md:hidden ${
menuOpen
?"translate-x-0"
:"translate-x-full"
}`}
...
className="menuBtn justify-between"
>
